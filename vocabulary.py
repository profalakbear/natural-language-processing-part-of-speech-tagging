...
#region Calculate frequency of each word
count=0
FREQUENCY_LIST_OF_EACH_WORD = []
MY_TUPLE = []

for i in range(len(array_of_tuple)):
  for j in range(len(array_of_tuple)):
    if array_of_tuple[i][0] == array_of_tuple[j][0]:
      count += 1
  MY_TUPLE = (array_of_tuple[i][0],count,array_of_tuple[i][1])
  FREQUENCY_LIST_OF_EACH_WORD.append(MY_TUPLE)
  count = 0
  MY_TUPLE = []
#endregion

df = pd.DataFrame(FREQUENCY_LIST_OF_EACH_WORD, columns =['   WORD', '   FREQUENCY', '   TAG'])
#print(df)

f = open("vocabulary.txt", "x")
f.write('WORD  FREQUENCY  TAG \n')
np.savetxt(r'C:\Users\alakb\Desktop\NLP HOMETASK POS TAGGING WITH HMM\final\vocabulary.txt', df.values, fmt='%s')
